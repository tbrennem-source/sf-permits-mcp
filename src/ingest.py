...
async def ingest_addenda(conn, client: SODAClient) -> int:
    """Ingest building permit addenda with routing into addenda table.

    Uses larger page size and periodic batch flushing for the ~3.9M row dataset.
    """
    print("\n=== Ingesting Building Permit Addenda ===")
    ...
    while True:
        ...
        for r in page:
            row_id += 1
            batch.append(_normalize_addendum(r, row_id))
    ...
    conn.execute(
        "INSERT OR REPLACE INTO ingest_log VALUES (?, ?, ?, ?, ?)",
        [endpoint_id, "Building Permit Addenda", datetime.now(timezone.utc).isoformat(), total, total],
    )
    elapsed = time.time() - start
