...
def test_normalize_addendum():
    ...
    result = _normalize_addendum(record, 1)
    assert result[0] == 1  # row_id
    assert result[1] == "2024-0001"  # application_number
    assert result[4] == "PLANNING"  # station
    assert result[10] == "Jane Doe"  # plan_checked_by
    assert result[12] == "CPC"  # department
    assert len(result) == 14


def test_normalize_addendum_empty_checker():
    record = {"application_number": "X", "plan_checked_by": "  "}
    result = _normalize_addendum(record, 1)
    assert result[10] is None  # empty string stripped to None
...
def test_extract_addenda_contacts(db_path):
    ...
    # Insert test addenda records
    conn.executemany(
        "INSERT INTO addenda VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
            (1, "P001", "01", "5", "PLANNING", "2024-01-15", None, None, None, None,
             "Jane Doe", "approved", "CPC", None),
            (2, "P001", "01", "6", "FIRE", "2024-01-16", None, None, None, None,
             "John Smith", "approved", "SFFD", None),
            (3, "P002", "01", "5", "PLANNING", "2024-02-01", None, None, None, None,
             "Jane Doe", "approved", "CPC", None),
            (4, "P003", "01", "5", "PLANNING", "2024-03-01", None, None, None, None,
             None, "pending", "CPC", None),  # NULL checker
        ],
    )
